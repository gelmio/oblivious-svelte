<script lang="ts">
  ...
  const carouselPhotos = [
    ...
    },
    {
      ...
      caption:
        "Hitchhiking the Sahara on a mining train into the night. Life goals.",
    },
    {
    ...
    },
    {
      ...
      caption:
        "One of the few properly done photo's from the trip. Props to our favourite Greek, Manos.",
    },
    {
    ...
    },
    {
      ...
      caption:
        "Hot as fuck in 40 degree heat and a leather jacket. The smile is a delerious one.",
    },
  ];

  let index = $state(0);

  const changePhoto = (step: number, stopInterval = false) => {
    if (index === 0 && step === -1) {
  ...
  $effect(() => {
    interval = setInterval(() => changePhoto(1), 6000);
...
<figure class="relative">
  <img
    class="relative"
    transition:fade
    src={carouselPhotos[index].imageSource}
    alt={carouselPhotos[index].caption}
  />
  <button
    class="absolute top-1/2 transform -translate-y-1/2 left-0 z-10 bg-transparent"
    onclick={() => changePhoto(-1, true)}
  >
    <img
      class="h-20 md:h-40 opacity-50"
      src="/images/icons/keyboard_arrow_left-24px.svg"
      alt="arrow left"
    />
  </button>
  <button
    class="absolute h-20 md:h-auto top-1/2 transform -translate-y-1/2 right-0 z-10 bg-transparent"
    onclick={() => changePhoto(1, true)}
  >
    <img
      class="h-20 md:h-40 opacity-50"
  ...
  <figcaption
    class="absolute top-100 left-0 right-0 text-center text-xl text-oblivious-dark"
  >
    {carouselPhotos[index].caption}
  </figcaption>
</figure>
