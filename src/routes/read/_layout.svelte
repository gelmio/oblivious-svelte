    ...
    <div class="rounded-lg bg-white p-2 md:p-16 flex flex-col text-center m-2">
      ...
      <div>
        <a
          ...
          >Sure, take me to chapter {$readerPosition[1]}</a
        >
        <span
          on:click={() => {
            recommendJumpToChapter = false;
...
<nav
  class="absolute z-10 top-16 md:top-32 left-0 h-4/5vh max-h-96 right-auto overflow-y-scroll transform {navOpen
    ? 'translate-x-0'
    ...
    {#if bookIndex + 1 < 4}
      ...
      <ul>
        {#each [...Array(chapterCount)] as _, chapterIndex}
          ...
          >
            <a rel="prefetch" href="read/{bookIndex + 1}/{chapterIndex + 1}"
