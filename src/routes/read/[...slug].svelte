<script context="module" lang="ts">
  export async function preload({ params, query }) {
    ...
    const res = await this.fetch(`read/${bookNumber}.json`);
    const { bookContents, message } = await res.json();
...
{#if $giveScrollHint}
  ...
  >
    <div class="rounded-lg bg-white p-2 md:p-16 flex flex-col text-center m-2">
      <p class="font-header text-xl md:text-2xl mb-4">Here's some tips:</p>
      <p class="font-sans text-base md:text-lg mb-1">
        Tap the text to turn the pages.
      </p>
      <p class="font-sans text-base md:text-lg mb-1">Tap the images to zoom.</p>
      <p class="font-sans text-base md:text-lg mb-1">
